...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-800">Edit Blog Category</h1>
</div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4" 
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $blogCategory->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="order" value="{{ old('order', $blogCategory->order) }}" 
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="space-y-4">
                    <div class="flex items-center">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $blogCategory->is_active) ? 'checked' : '' }}
            ...
            <div class="bg-gray-50 rounded-lg p-6 border border-gray-200">
                ...
                <div class="space-y-2 text-xs text-gray-600">
                    <div class="flex justify-between">
                        <span>Posts:</span>
                        <span class="font-semibold">{{ $blogCategory->posts()->count() }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span>Created:</span>
                        <span>{{ $blogCategory->created_at->format('M d, Y') }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span>Updated:</span>
                        <span>{{ $blogCategory->updated_at->format('M d, Y') }}</span>
                    </div>
                </div>
            </div>
        </div>
