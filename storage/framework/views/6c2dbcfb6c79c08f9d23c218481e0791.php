...
<div class="bg-white rounded-lg shadow-md p-6">
    <div class="flex items-center justify-between mb-6">
        <div class="flex gap-2">
            <a href="?" class="px-4 py-2 rounded-lg transition <?php echo e(!request('type') ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'); ?>">
                <i class="fas fa-th mr-2"></i> All
            </a>
            <a href="?type=image" class="px-4 py-2 rounded-lg transition <?php echo e(request('type') == 'image' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'); ?>">
                <i class="fas fa-image mr-2"></i> Images
            </a>
            <a href="?type=video" class="px-4 py-2 rounded-lg transition <?php echo e(request('type') == 'video' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'); ?>">
                <i class="fas fa-video mr-2"></i> Videos
            </a>
            <a href="?type=document" class="px-4 py-2 rounded-lg transition <?php echo e(request('type') == 'document' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'); ?>">
                <i class="fas fa-file mr-2"></i> Documents
            </a>
        </div>
        <div class="text-sm text-gray-600">
            <span class="font-semibold"><?php echo e($media->total()); ?></span> files
        </div>
    </div>
            ...
            <?php $__currentLoopData = $media; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="relative group border rounded-lg overflow-hidden hover:shadow-lg transition">
                    <?php if($item->type == 'image'): ?>
                        <img src="<?php echo e(asset('storage/' . $item->file_path)); ?>" alt="<?php echo e($item->name); ?>" class="w-full h-32 object-cover">
                    ...
                    <div class="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-2">
                        ...
                        <button onclick="copyUrl('<?php echo e(asset('storage/' . $item->file_path)); ?>')" 
                            class="p-2 bg-white rounded-full text-green-600 hover:bg-green-600 hover:text-white transition">
                            <i class="fas fa-copy"></i>
                        </button>
                        <button onclick="deleteMedia(<?php echo e($item->id); ?>)" 
                    ...
                    <div class="p-2 bg-white">
                        <p class="text-xs text-gray-600 truncate" title="<?php echo e($item->name); ?>"><?php echo e($item->name); ?></p>
                        <p class="text-xs text-gray-500"><?php echo e(number_format($item->size / 1024, 1)); ?> KB</p>
                    </div>
                </div>
...
function deleteMedia(id) {
    if (confirm('Are you sure you want to delete this file?')) {
        fetch(`<?php echo e(route("admin.media.destroy", '')); ?>/${id}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
...
<?php echo $__env->make('admin.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/macbook/Documents/Projects/New_heros/resources/views/admin/media/index.blade.php ENDPATH**/ ?>